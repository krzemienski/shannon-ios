...
echo "=========================================="
echo "Fixing ClaudeCode iOS Compilation Errors"
echo "=========================================="

# Fix ToolStore.swift
echo "Fixing ToolStore.swift..."
cat > Sources/Core/State/ToolStore.swift << 'EOF'
//
//  ToolStore.swift
//  ClaudeCode
//
...
public final class ToolStore: ObservableObject {
    ...
    public func cancelExecution(_ executionId: String) {
...
public struct Tool: Identifiable, Equatable {
    ...
    public let category: ToolStoreCategory
...
if [ -f "Sources/Services/NetworkMonitor.swift" ]; then
    sed -i '' 's/private init()/public init()/' Sources/Services/NetworkMonitor.swift
fi

# Fix OfflineQueueManager
echo "Fixing OfflineQueueManager..."
if [ -f "Sources/Services/OfflineQueueManager.swift" ]; then
    sed -i '' 's/private init()/public init()/' Sources/Services/OfflineQueueManager.swift
fi

echo "✅ Compilation error fixes complete!"
echo ""
echo "Next steps:"
echo "1. Run: ./Scripts/simulator_automation.sh build"
